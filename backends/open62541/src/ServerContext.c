...
AddNodeContext *
AddNodeContext_new(UA_Server *server) {
    ...
    // Initialize the container for nodes that could not be added right away
    NodeContainer_init(&ctx->problemNodes, 64, false);
    ...
    // Set the server-logger as the default
    UA_ServerConfig *config = UA_Server_getConfig(server);
    ctx->logger = config->logging;
...
void AddNodeContext_delete(AddNodeContext *ctx) {
    ...
    NodeContainer_clear(&ctx->problemNodes);
    if(ctx->customTypes) {
