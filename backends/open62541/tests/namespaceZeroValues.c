...
START_TEST(Server_LoadNS0Values)
{
    ...
    ck_assert(
        UA_QualifiedName_equal(&qualifiedName, (UA_QualifiedName *)var.data));
    UA_Variant_clear(&var);
    // NodeId
...
START_TEST(EnumValues)
{
    UA_Variant var;
    UA_StatusCode retval =
        UA_Server_readValue(server, UA_NODEID_NUMERIC(2, 1010), &var);
    ck_assert_uint_eq(retval, UA_STATUSCODE_GOOD);
    ...
    ck_assert(!strncmp(enumVal[0].displayName.text.data, "LOG_ON",
                       enumVal[0].displayName.text.length));
...
START_TEST(NumericRange)
{
    ...
    ck_assert(!strncmp(numericRange[0].data, "1:65535",
numericRange[0].length)); UA_Variant_clear(&var);
}
END_TEST
*/
