...
void PrintValueMember(const void *const pData, UA_UInt16 MemberTypeIndex,
                      const UA_UInt32 Indentation, ostream &out)
{
    string strIndent = string(Indentation, '\t');

    out << "\t\t MemberTypeIndex = " << MemberTypeIndex << endl;
    switch (MemberTypeIndex)
    {
    case UA_TYPES_BOOLEAN:
        out << strIndent << ((*(UA_Boolean *)pData) ? "true" : "false") << endl;
        break;
    case UA_TYPES_BYTE:
    case UA_TYPES_UINT16:
    case UA_TYPES_UINT32:
    case UA_TYPES_UINT64:
        out << strIndent << (*(UA_UInt64 *)pData) << endl;
        break;
    case UA_TYPES_SBYTE:
    case UA_TYPES_INT16:
    case UA_TYPES_INT32:
    case UA_TYPES_INT64:
        out << strIndent << (*(UA_Int64 *)pData) << endl;
        break;
    case UA_TYPES_STRING: // TODO: derived types?
    case UA_TYPES_BYTESTRING:
        out << strIndent << string((char *)pData) << endl;
    default:
        out << strIndent << "Not supported" << endl;
        break;
    }
}
...
void PrintValueAttribute(const UA_Variant &Value, ostream &out)
{
    ...
    // TODO: variants can be empty, then the pointer to the type description is
    // null
    if (Value.type != 0)
    {

        // out << "\tType Id = " << Value.type->typeId;
        out << "\tStorage Type = " << Value.storageType;
        PrintArrayDimensions(Value.arrayDimensionsSize, Value.arrayDimensions,
                             1, out);
        out << "\tArray Length = " << Value.arrayLength;

        // a variant can only have members of BaseDataType

        for (size_t i = 0; i < Value.type->membersSize; i++)
        {

            /* PrintValueMember(Value.data,
               Value.type->members[i].memberTypeIndex, 1, out);
                             */
        }
    }
