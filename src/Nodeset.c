...
static const NodeAttribute dataTypeDefinition_IsOptionSet = {"IsOptionSet",
                                                             "false"};
...
static enum ZIP_CMP compareNodeId(const void *a, const void *b)
{
    return (enum ZIP_CMP)UA_NodeId_order((const UA_NodeId *)a,
                                         (const UA_NodeId *)b);
}

ZIP_FUNCTIONS(NodeTree, NL_Node, treeEntry, UA_NodeId, id, compareNodeId)

static void NodeList_append(NodeList *list, NL_Node *node)
{
    node->sortNext = NULL;
    if (list->tail)
...
static NL_Node *Node_new(NL_NodeClass nodeClass)
{
    ...
    if ((unsigned)nodeClass >= NL_NODECLASS_COUNT)
        return NULL;
    return (NL_Node *)UA_calloc(1, nodeSizes[nodeClass]);
}

static void Node_delete(NL_Node *node)
{
    if (!node)
        return;
...
static AliasList *AliasList_new(void)
{
    return (AliasList *)UA_calloc(1, sizeof(AliasList) +
                                         MAX_ALIAS * sizeof(Alias));
}

static const UA_NodeId *AliasList_getNodeId(const AliasList *list,
                                            const char *name)
{
    if (!name)
        return NULL;
    for (const Alias *alias = list->data; alias != list->data + list->size;
         alias++)
    {
...
static void AliasList_delete(AliasList *list)
{
    if (!list)
        return;
...
static bool parseNodeId(const Nodeset *nodeset, char *s, UA_NodeId *out)
{
    if (!s)
        return false;
    return UA_NodeId_parseEx(out, UA_STRING(s),
                             &nodeset->context->namespaceMapping) ==
           UA_STATUSCODE_GOOD;
}

static bool parseQualifiedName(const Nodeset *nodeset, char *s,
                               UA_QualifiedName *out)
{
    if (!s)
        return false;
    UA_StatusCode res = UA_QualifiedName_parseEx(
        out, UA_STRING(s), &nodeset->context->namespaceMapping);
    if (res != UA_STATUSCODE_GOOD)
        return false;
    out->namespaceIndex = UA_NamespaceMapping_remote2Local(
        &nodeset->context->namespaceMapping, out->namespaceIndex);
    return true;
}

static bool alias2Id(const Nodeset *nodeset, char *name, UA_NodeId *out)
{
...
Nodeset *UA_NodeSet_new(UA_NodeSetLoaderContext *context)
{
    ...
    {
        UA_NodeSet_cleanup(nodeset);
        return NULL;
    }
    ZIP_INIT(&nodeset->nodeTree);
    nodeset->context = context;
    return nodeset;
}
...
bool UA_NodeSet_ownTextBuffer(Nodeset *nodeset, char *data)
{
    NodesetTextBuffer *buffer =
        (NodesetTextBuffer *)UA_malloc(sizeof(NodesetTextBuffer));
    if (!buffer)
        return false;
    buffer->data = data;
...
static NL_Node *findByNodeId(Nodeset *nodeset, const UA_NodeId *key)
{
...
static bool nodeDependenciesReady(Nodeset *nodeset, NL_Node *node)
{
    ...
    {
        NL_Node *memberType =
            findByNodeId(nodeset, &dataType->definition->fields[i].dataType);
...
static bool sortNodeClass(Nodeset *nodeset, NL_NodeClass nodeClass)
{
...
static bool sortNodeClassLogged(Nodeset *nodeset, NL_NodeClass nodeClass,
                                const char *description)
{
    bool done = sortNodeClass(nodeset, nodeClass);
    if (!done)
        UA_LOG_ERROR(nodeset->context->logger, UA_LOGCATEGORY_SERVER,
                     "NodeSetLoader: Cannot add %s", description);
    return done;
}

static void *resolveReferenceTargets(void *context, NL_Node *node)
{
    Nodeset *nodeset = (Nodeset *)context;
    for (NL_Reference *ref = node->refs; ref; ref = ref->next)
        ref->targetPtr = findByNodeId(nodeset, &ref->target);
    return NULL;
}
...
bool UA_NodeSet_sort(Nodeset *nodeset)
{
    ...
    // If the target is not found in the tree, assume it already exists in the
    // server.
    ZIP_ITER(NodeTree, &nodeset->nodeTree, resolveReferenceTargets, nodeset);

    bool allDone = sortNodeClassLogged(nodeset, NODECLASS_REFERENCETYPE,
                                       "ReferenceType hierarchy");
    allDone &=
        sortNodeClassLogged(nodeset, NODECLASS_DATATYPE, "DataType hierarchy");
    allDone &= sortNodeClassLogged(nodeset, NODECLASS_VARIABLETYPE,
                                   "VariableType hierarchy");
    allDone &= sortNodeClassLogged(nodeset, NODECLASS_VIEW, "Views");
    ...
    do
    {
        ...
        done &= sortNodeClass(nodeset, NODECLASS_OBJECTTYPE);
        done &= sortNodeClass(nodeset, NODECLASS_OBJECT);
        done &= sortNodeClass(nodeset, NODECLASS_METHOD);
        done &= sortNodeClass(nodeset, NODECLASS_VARIABLE);
...
static void *deleteNode(void *context, NL_Node *node)
{
    (void)context;
    Node_delete(node);
    return NULL;
...
static char *getAttributeValue(const NodeAttribute *attr,
...
static bool extractAttributes(Nodeset *nodeset, NL_Node *node,
                              const XmlAttributes *attributes)
{
    ...
    case NODECLASS_VARIABLE: {
        ...
        if (!alias2Id(nodeset, datatype, &((NL_VariableNode *)node)->datatype))
            return false;
                             ...
                             const XmlAttributes *attributes, char *idString)
{
    ...
    char *isForwardString = getAttributeValue(&attrIsForward, attributes);
    newRef->isForward = !strcmp("true", isForwardString);

    char *aliasIdString = getAttributeValue(&attrReferenceType, attributes);
...
DataTypeNode_addDefinitionField(NL_DataTypeDefinition *def)
{
    ...
    NL_DataTypeDefinitionField *fields =
        (NL_DataTypeDefinitionField *)UA_realloc(
                                      ...
                                      const XmlAttributes *attributes)
{
    ...
    if (dataTypeNode->definition)
        return false;
    dataTypeNode->definition =
        (NL_DataTypeDefinition *)UA_calloc(1, sizeof(NL_DataTypeDefinition));
    if (!dataTypeNode->definition)
        return false;
    dataTypeNode->definition->isUnion = !strcmp(
        "true", getAttributeValue(&dataTypeDefinition_IsUnion, attributes));
    dataTypeNode->definition->isOptionSet = !strcmp(
        "true", getAttributeValue(&dataTypeDefinition_IsOptionSet, attributes));
    return true;
}
                                 ...
                                 const XmlAttributes *attributes)
{
    ...
    else
    {
        ...
        newField->valueRank =
            atoi(getAttributeValue(&attrValueRank, attributes));
        char *isOptional =
...
bool UA_NodeSet_addNamespace(Nodeset *nodeset, char *namespaceUri)
{
    ...
    return UA_NodeSetLoaderContext_addNamespace(nodeset->context, uri, false) ==
           UA_STATUSCODE_GOOD;
}
...
{
    target->locale = UA_STRING(getAttributeValue(&attrLocale, attributes));
