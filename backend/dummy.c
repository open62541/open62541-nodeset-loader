...
void addNode(const TNode* node)
{
    ...
    switch (node->nodeClass)
    {
        ...
        case NODECLASS_VARIABLE:
            printf("\tparentNodeId: %s\n", ((TVariableNode *)node)->parentNodeId.idString);
    ...
    {
        printf("\treftype: %s target: %s\n", hierachicalRef->refType.idString, hierachicalRef->target.idString);
    ...
    while (nonHierRef)
    {
        printf("\treftype: %s target: %s\n", nonHierRef->refType.idString, nonHierRef->target.idString);
